...
func (ud *userUseCase) AddUser(newUser domain.User) (domain.User, error) {
	...
	hashed, err := bcrypt.GenerateFromPassword([]byte(newUser.Pin), bcrypt.DefaultCost)
	if err != nil {
		return domain.User{}, err
	}
	newUser.Pin = string(hashed)
...
func (ud *userUseCase) UpdateUser(id int, updateProfile domain.User) (domain.User, error) {
	...
	hashed, err := bcrypt.GenerateFromPassword([]byte(updateProfile.Pin), bcrypt.DefaultCost)

	if err != nil {
		log.Println("error encrypt pin", err)
		return domain.User{}, err
	}
	updateProfile.Pin = string(hashed)
